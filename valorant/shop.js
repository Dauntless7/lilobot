...
export const getShop = async (id, account = null) => {
  ...
  console.assert(
    req.statusCode === 200,
    `Valorant skins offers code is ${req.statusCode}!`,
    req
  );
  ...
  Promise.all(
    json.FeaturedBundle.Bundles.map((rawBundle) => formatBundle(rawBundle))
  ).then(async (bundles) => {
...
let nextNMTimestamp = null,
  nextNMTimestampUpdated = 0;
export const getNextNightMarketTimestamp = async () => {
  ...
  if (isNaN(nextNMTimestamp) || nextNMTimestamp < Date.now() / 1000)
...
export const getShopCache = (puuid, target = 'offers', print = true) => {
  ...
  try {
    const shopCache = JSON.parse(
      fs.readFileSync('data/shopCache/' + puuid + '.json', 'utf8')
    );

    let expiresTimestamp;
    if (target === 'offers') expiresTimestamp = shopCache[target].expires;
    else if (target === 'night_market')
      expiresTimestamp = shopCache[target]
        ? shopCache[target].expires
        : getMidnightTimestamp(shopCache.timestamp);
    else if (target === 'bundles')
      expiresTimestamp = Math.min(
        ...shopCache.bundles.map((bundle) => bundle.expires),
        get9PMTimetstamp(Date.now())
      );
    else if (target === 'all')
      expiresTimestamp = Math.min(
        shopCache.offers.expires,
        ...shopCache.bundles.map((bundle) => bundle.expires),
        get9PMTimetstamp(Date.now()),
        shopCache.night_market.expires
      );
...
const addShopCache = (puuid, shopJson) => {
  ...
  const shopCache = {
    offers: {
      ...
      accessory: {
        offers: (shopJson.AccessoryStore.AccessoryStoreOffers || []).map(
          (rawAccessory) => {
            return {
        ...
        expires:
          Math.floor(now / 1000) +
          shopJson.AccessoryStore.AccessoryStoreRemainingDurationInSeconds
      }
    },
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(shopCache, null, 2)
  );
