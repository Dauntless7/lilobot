...
const getCache = (user, type) => {
  if (user.puuid in xCache[type]) {
    ...
    const expiresIn =
      cached.timestamp - Date.now() + config.careerCacheExpiration;
    if (expiresIn <= 0) {
      delete xCache[type][user.puuid];
      return { success: false };
    } else {
      console.log(
        `Fetched ${type} from cache for user ${user.username}! It expires in ${Math.ceil(expiresIn / 1000)}s.`
      );
...
export const getAccountInfo = async (user, interaction) => {
  ...
  if (progress(user, 'acc') || progress(user, 'mmr'))
    return {
      success: false,
      error: s(interaction).error.WAIT_FOR_PREVIUS_REQUEST
    };
  const accountData = await VAPI.getAccountByPUUID({
    puuid: user.puuid,
    force: true
  });
  console.log(
    `Checked ACCData for ${user.id} R:${accountData?.ratelimits?.remaining} Reset: in ${accountData?.ratelimits?.reset} seconds`
  );
  ...
  console.log(
    `Checked MMRData for ${user.id} R:${mmrData?.ratelimits?.remaining} Reset: in ${mmrData?.ratelimits?.reset} seconds`
  );
  ...
  const mmr = {
    current_data: mmrData.data.current_data,
    highest_rank: mmrData.data.highest_rank
  };
...
export const fetchMatchHistory = async (
  ...
  if (
    progress(user, 'matches') ||
    (mode === 'competitive' && progress(user, 'mmrHistory'))
  )
    return {
      success: false,
      error: s(interaction).error.WAIT_FOR_PREVIUS_REQUEST
    };
  const matchHistory = await VAPI.getMatchesByPUUID({
    puuid: user.puuid,
    region: user.region,
    filter: mode
  });
  console.log(
    `Checked match history for ${user.id} R:${matchHistory?.ratelimits?.remaining} Reset: in ${matchHistory?.ratelimits?.reset} seconds`
  );
  setTimeout(() => {
    progress(user, 'matches', true);
  }, 5000);
  if (matchHistory.error)
    return { success: false, error: matchHistory.error[0].message };
  else if (matchHistory.data.length === 0)
    return {
      success: false,
      error: s(interaction).error.NO_MATCH_DATA.f({ m: mode })
    };
  let mmrHistory;
  if (mode === 'competitive') {
    mmrHistory = await VAPI.getMMRHistoryByPUUID({
      puuid: user.puuid,
      region: user.region
    });
    console.log(
      `Checked MMRHistory for ${user.id} R:${mmrHistory?.ratelimits?.remaining} Reset: in ${mmrHistory?.ratelimits?.reset} seconds`
    );
    setTimeout(() => {
      progress(user, 'mmrHistory', true);
    }, 1000);
    if (mmrHistory.error)
      return { success: false, error: mmrHistory.error[0].message };
    const matches = [];

    for (let i = 0; i < matchHistory.data.length; i++) {
      const match = matchHistory.data[i];
      const data = { metadata: {}, player: {}, teams: {} };
      const matchMMR = mmrHistory.data.find(
        (item) => item.match_id === match.metadata.matchid
      );
      ...
      const playerPosition =
        match.players.all_players
          ...
          .findIndex((player) => player.puuid === user.puuid) + 1;

      data.player.hs_percent = Math.ceil(
      ...
      data.player.average_damage_round = (
        player.damage_made / match.metadata.rounds_played
      ).toFixed(1);
      data.player.average_combat_score = (
        player.stats.score / match.metadata.rounds_played
      ).toFixed(1);
      data.player.agent = {
        name: player.character,
        iconUrl: player.assets.agent.small
      };
      ...
      data.metadata.pt_round_won =
        match.teams[player.team.toLowerCase()].rounds_won;
      data.metadata.et_round_won =
        match.teams[player.team.toLowerCase()].rounds_lost;
