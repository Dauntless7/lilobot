...
export const getEntitlements = async (user, itemTypeId, itemType = 'item') => {
  ...
  console.assert(
    req.statusCode === 200,
    `Valorant ${itemType} entitlements code is ${req.statusCode}!`,
    req
  );
...
export const getSkins = async (user) => {
  ...
  if (user.puuid in skinCache) {
    ...
    const expiresIn =
      cached.timestamp - Date.now() + config.loadoutCacheExpiration;
    if (expiresIn <= 0) {
...
export const getLoadout = async (user, account) => {
  ...
  console.assert(
    req.statusCode === 200,
    `Valorant loadout fetch code is ${req.statusCode}!`,
    req
  );
  ...
  console.assert(
    req.statusCode === 200,
    `Valorant favorites fetch code is ${req.statusCode}!`,
    req
  );
  ...
  if (json2.httpStatus === 400 && json2.errorCode === 'BAD_CLAIMS') {
    deleteUserAuth(user);
    return { success: false };
  } else if (isMaintenance(json2)) return { success: false, maintenance: true };
...
export const renderCollection = async (
  ...
  if (weaponName)
    return await renderCollectionOfWeapon(interaction, targetId, weaponName);

  const loadout = await getLoadout(user);
  if (!loadout.success)
    return errorFetchingCollection(loadout, interaction, targetId);
...
const renderCollectionOfWeapon = async (interaction, targetId, weaponName) => {
  ...
  if (!skins.success)
    return errorFetchingCollection(skins, interaction, targetId);

  return await collectionOfWeaponEmbed(
    interaction,
    targetId,
    user,
    WeaponTypeUuid[weaponName],
    skins.skins
  );
};
...
const errorFetchingCollection = (result, interaction, targetId) => {
  if (!result.success) {
    ...
    if (targetId && targetId !== interaction.user.id)
      errorText = s(interaction).error.AUTH_ERROR_COLLECTION_OTHER.f({
        u: `<@${targetId}>`
      });
