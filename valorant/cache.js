...
export const fetchBattlepassInfo = async (gameVersion) => {
  ...
  console.assert(
    json1.status === 200,
    `Valorant seasons data status code is ${json1.status}!`,
    json1
  );
  ...
  console.assert(
    json2.status === 200,
    `Valorant contracts data status code is ${json2.status}!`,
    json2
  );

  // find current season
  const now = Date.now();
  const currentSeason = json1.data.find(
    (season) =>
      season.type === 'EAresSeasonType::Act' &&
      new Date(season.startTime) < now &&
      new Date(season.endTime) > now
  );

  // find current battlepass
  const currentBattlepass = json2.data.find(
    (contract) => contract.content.relationUuid === currentSeason.uuid
  );
...
export const searchBundle = async (
  query,
  locale,
  limit = 20,
  threshold = -1000
