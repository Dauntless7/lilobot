...
export const queueItemShop = async (id) => {
  if (!config.useShopQueue || shopCached(id, 'offers') || useMultiqueue())
    return { inQueue: false, ...(await getOffers(id)) };
...
export const queueNightMarket = async (id) => {
  if (!config.useShopQueue || shopCached(id, 'night_market') || useMultiqueue())
    return { inQueue: false, ...(await getNightMarket(id)) };
  const c = queueCounter++;
  queue.push({
...
export const queueBundles = async (id) => {
  if (!config.useShopQueue || shopCached(id, 'bundles') || useMultiqueue())
    return { inQueue: false, ...(await getBundles(id)) };
  const c = queueCounter++;
  queue.push({
...
export const queueShop = async (id, account = null) => {
  if (!config.useShopQueue || shopCached(id) || useMultiqueue())
    return { inQueue: false, ...(await getOffers(id, account)) };
  const c = queueCounter++;
  queue.push({
    operation: Operations.BUNDLES,
    c,
...
export const queueNullOperation = async (timeout) => {
  // used for stress-testing the shop queue
  ...
  console.log(
    `Added null operation to shop queue with timeout ${timeout} (c=${c})`
  );
...
export const processShopQueue = async () => {
  ...
  console.log(
    `Processing shop queue item "${item.operation}" for ${item.id} (c=${item.c})`
  );
  processingCount++;
  ...
  try {
    ...
    console.error(
      `Error processing shop queue item "${item.operation}" for ${item.id} (c=${item.c})`
    );
    console.error(e);
  ...
  console.log(
    `Finished processing shop queue item "${item.operation}" for ${item.id} (c=${item.c})`
  );
  processingCount--;
};
