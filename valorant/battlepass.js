...
const getWeeklies = async () => {
  ...
  console.assert(
    req.statusCode === 200,
    `Valorant mission status code is ${req.statusCode}!`,
    req
  );
  ...
  console.assert(
    json.status === 200,
    `Valorant mission data status code is ${json.status}!`,
    json
  );
  ...
  json['data'].forEach((mission) => {
    if (
      mission.type === 'EAresMissionType::Weekly' &&
      new Date(mission.expirationDate) > now
    ) {
...
export const getBattlepassProgress = async (
  interaction,
  maxlevel,
  ...
  console.assert(
    req.statusCode === 200,
    `Valorant battlepass code is ${req.statusCode}!`,
    req
  );
  ...
  const battlepassInfo = await getBattlepassInfo();
  const contract = json.Contracts.find(
    (contract) => contract.ContractDefinitionID === battlepassInfo.uuid
  );
  ...
  if (battlepassPurchased.success === false)
    // login failed
  ...
  return {
    ...
    nextReward: await getNextReward(
      interaction,
      contractData.progressionLevelReached
    ),
    season_days_left: season_days_left,
    totalxp: totalxp.toLocaleString(),
    xpneeded: (
      (await calculate_level_xp(contractData.progressionLevelReached + 1)) -
      contractData.progressionTowardsNextLevel
    ).toLocaleString(),
    totalxpneeded: Math.max(0, totalxpneeded).toLocaleString(),
    weeklyxp: weeklyxp.toLocaleString(),
    spikerushneeded: Math.max(
      0,
      Math.ceil(totalxpneeded / spikerush_xp)
    ).toLocaleString(),
    normalneeded: Math.max(
      0,
      Math.ceil(totalxpneeded / average_unrated_xp)
    ).toLocaleString(),
    spikerushneededwithweeklies: Math.max(
      0,
      Math.ceil((totalxpneeded - weeklyxp) / spikerush_xp)
    ).toLocaleString(),
    normalneededwithweeklies: Math.max(
      0,
      Math.ceil((totalxpneeded - weeklyxp) / average_unrated_xp)
    ).toLocaleString(),
    dailyxpneeded: Math.max(
      0,
      Math.ceil(totalxpneeded / season_days_left)
    ).toLocaleString(),
    weeklyxpneeded: Math.max(
      0,
      Math.ceil(totalxpneeded / season_weeks_left)
    ).toLocaleString(),
    dailyxpneededwithweeklies: Math.max(
      0,
      Math.ceil((totalxpneeded - weeklyxp) / season_days_left)
    ).toLocaleString(),
    weeklyxpneededwithweeklies: Math.max(
      0,
      Math.ceil((totalxpneeded - weeklyxp) / season_weeks_left)
    ).toLocaleString()
  };
};
  ...
  if (userMissionsObj.missionArray.length > 2) {
    userMissionsObj.missionArray.forEach((userMission) => {
      if (!userMission.Complete) {
        Object.entries(seasonWeeklyMissions).forEach(
          ([date, weeklyMissions]) => {
            Object.entries(weeklyMissions).forEach(([uuid, missionDetails]) => {
              if (uuid === userMission.ID) {
                xp = xp + missionDetails.xpGrant;
                userMissionsObj.weeklyCheckpoint =
                  missionDetails.activationDate; // update checkpoint to prevent adding this weeks XP later on
              }
            });
          }
        );
...
export const renderBattlepassProgress = async (
  interaction,
  targetId = interaction.user.id
) => {
  const maxlevel =
    (interaction.options && interaction.options.getInteger('maxlevel')) || 50;
  const battlepassProgress = await getBattlepassProgress(
    interaction,
    maxlevel,
    targetId
  );

  return await renderBattlepass(
    battlepassProgress,
    maxlevel,
    interaction,
    targetId
