...
export const alertsForUser = (id, account = null) => {
  if (account === -1) {
    // -1 to get all alerts for user across accounts
...
export const filteredAlertsForUser = async (interaction) => {
  ...
  const alertPriority = (alert) => {
    ...
    if (
      interaction.guild &&
      channel &&
      channel.client.channels.cache.get(alert.channel_id).guildId ===
  ...
  alerts.sort(
    (alert1, alert2) => alertPriority(alert2) - alertPriority(alert1)
  );
...
export const alertsPerChannelPerGuild = async () => {
  ...
  for (const id of getUserList()) {
    ...
    for (const alert of alerts) {
      ...
      if (!(alert.channel_id in guilds[guildId]))
        guilds[guildId][alert.channel_id] = 1;
    ...
    for (const id of getUserList()) {
      try {
        ...
        for (let i = 1; i <= accountCount; i++) {
          ...
          console.log(
            `Checking user ${discordUsername}'s ${valorantUser.username} account (${i}/${accountCount}) for alerts...`
          );
          ...
          let offers;
          do {
            // retry loop in case of rate limit or maintenance
            ...
            if (!offers.success) {
              if (offers.maintenance) {
                console.log(
                  'Valorant servers are under maintenance, waiting 15min before continuing alert checks...'
                );
        ...
        console.error(
          'There was an error while trying to fetch and send alerts for user ' +
            discordTag(id)
        );
        console.error(e);
      }
...
export const sendCredentialsExpired = async (
  id,
  alert,
  tryOnOtherShard = true
) => {
  ...
  if (!channel) {
    ...
    if (user)
      console.error(
        `Please tell ${user.tag} that their credentials have expired, and that they should /login again. (I can't find the channel where the alert was set up)`
      );
    return;
  }
  ...
  await channel
    .send({
      ...
      embeds: [
        {
          description: s(valorantUser).error.AUTH_ERROR_ALERTS_HAPPENED.f({
            u: id
          }),
          color: VAL_COLOR_1
        }
    ...
    .catch(async (e) => {
      console.error(
        `Could not send message in #${channel.name}! Do I have the right role?`
      );
      ...
      try {
        ...
        if (user)
          console.error(
            `Please tell ${user.tag} that their credentials have expired, and that they should /login again. Also tell them that they should fix their perms.`
          );
      } catch (e) {}
...
export const testAlerts = async (interaction) => {
  try {
    const channel =
      interaction.channel || (await fetchChannel(interaction.channel_id));
    await channel.send({
    ...
    if (e.code === 50013) console.error("Failed with 'Missing Access' error");
      ...
      console.error("Failed with 'Missing Permissions' error");
