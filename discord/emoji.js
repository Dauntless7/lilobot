...
export const VPEmoji = async (interaction, channel = interaction.channel) =>
  emojiToString(
    await getOrCreateEmoji(channel, VPEmojiName, VPEmojiFilename)
  ) || s(interaction).info.PRICE;
export const RadEmoji = async (interaction, channel = interaction.channel) =>
  emojiToString(
    await getOrCreateEmoji(channel, RadEmojiName, RadEmojiFilename)
  );
export const KCEmoji = async (interaction, channel = interaction.channel) =>
  emojiToString(await getOrCreateEmoji(channel, KCEmojiName, KCEmojiFilename));

export const rarityEmoji = async (channel, name, icon) =>
  emojiToString(await getOrCreateEmoji(channel, `${name}Rarity`, icon));
...
const getOrCreateEmoji = async (channel, name, filenameOrUrl) => {
  ...
  if (externalAllowed) {
    if (config.useEmojisFromServer) {
      try {
        const emojiGuild = await client.guilds.fetch(
          config.useEmojisFromServer
        );
        if (!emojiGuild)
          console.error(
            'useEmojisFromServer server not found! Either the ID is incorrect or I am not in that server anymore!'
          );
        else {
    ...
    if (client.shard) {
      const results = await channel.client.shard.broadcastEval(findEmoji, {
        context: { name }
      });
  ...
  if (guild)
    return addEmojiToCache(await createEmoji(guild, name, filenameOrUrl));
};
...
const createEmoji = async (guild, name, filenameOrUrl) => {
  ...
  if (!canCreateEmojis(guild))
    return console.log(
      `Don't have permission to create emoji ${name} in guild ${guild.name}!`
    );

  await updateEmojiCache(guild);
  if (guild.emojis.cache.filter((e) => !e.animated).size >= maxEmojis(guild))
    return console.log(
      `Emoji limit of ${maxEmojis(guild)} reached for ${guild.name} while uploading ${name}!`
    );
...
const updateEmojiCache = async (guild) => {
  ...
  if (Date.now() - lastEmojiFetch[guild.id] < config.emojiCacheExpiration)
    return; // don't update emoji cache multiple times per second
...
const findEmoji = (c, { name }) => {
  return (
    c.emojis.cache.get(name) ||
    c.emojis.cache.find((e) => e.name.toLowerCase() === name.toLowerCase())
  );
};
