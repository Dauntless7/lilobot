...
export const skinChosenEmbed = async (interaction, skin) => {
  ...
  if (config.fetchSkinPrices && !skin.price)
...
export const renderOffers = async (
  shop,
  interaction,
  valorantUser,
  VPemoji,
  ...
  if (config.notice && valorantUser) {
    ...
    if (
      ...
      if (typeof config.notice === 'string') {
        if (config.notice.startsWith('{'))
          embeds.push(EmbedBuilder.from(JSON.parse(config.notice)).toJSON());
...
export const renderAccessoryOffers = async (
  shop,
  interaction,
  valorantUser,
  KCemoji
  ...
  let headerText = s(interaction).info.ACCESSORY_SHOP_HEADER.f(
...
export const getSkinLevels = async (
  offers,
  interaction,
  nightmarket = false
) => {
...
export const renderNightMarket = async (
  market,
  interaction,
  valorantUser,
  emoji
...
export const renderBattlepass = async (
  battlepass,
  targetlevel,
  interaction
) => {
    ...
    embeds.push(
      {
        ...
        description: `${s(interaction).battlepass.TIER_HEADER.f(
          { u: user.username },
          interaction
        )}\n${createProgressBar(
          battlepass.xpneeded,
...
export const skinCollectionSingleEmbed = async (
  interaction,
  id,
  user,
  { loadout, favorites }
  ...
  const createField = async (weaponUuid, inline = true) => {
    ...
    const skin = await getSkinFromSkinUuid(
      loadout.Guns.find((gun) => gun.ID === weaponUuid).SkinID
    );
    skinsUuid.push(skin);
  ...
  if (!someoneElseUsedCommand)
    components.push(...switchAccountButtons(interaction, 'cl', false, id));
...
export const skinCollectionPageEmbed = async (
  interaction,
  id,
  user,
  { loadout, favorites },
  ...
  const embeds = [
    basicEmbed(s(interaction).info.COLLECTION_HEADER.f({ u: usernameText }, id))
  ];
...
export const collectionOfWeaponEmbed = async (
  interaction,
  id,
  user,
  weaponTypeUuid,
  ...
  if (!someoneElseUsedCommand)
    actionRows.push(
      ...switchAccountButtons(interaction, `clw-${weaponTypeIndex}`, false, id)
    );
...
export const switchAccountButtons = (
  interaction,
  customId,
  ...
  const accountNumbers = [...Array(json.accounts.length).keys()]
    .map((n) => n + 1)
    .slice(0, config.maxAccountsPerUser <= 10 ? config.maxAccountsPerUser : 10);
...
export const alertsPageEmbed = async (
  interaction,
  alerts,
  pageIndex,
  emojiString
...
export const allStatsEmbed = async (interaction, stats, pageIndex = 0) => {
  ...
  const skinsToDisplay = Object.keys(stats.items).slice(
    pageIndex * config.statsPerPage,
    pageIndex * config.statsPerPage + config.statsPerPage
  );
  const embeds = [
    basicEmbed(
...
export const valMaintenancesEmbeds = (
  interaction,
...
export const valMaintenanceEmbed = (
  interaction,
  target,
  isIncident,
  regionName
