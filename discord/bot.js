...
<<<<<<< HEAD
  ...
  if (config.useLoginQueue && config.loginQueue)
    cronTasks.push(cron.schedule(config.loginQueue, processAuthQueue));

  // if shop queue is enabled, process an item every second
  if (config.useShopQueue && config.shopQueue)
    cronTasks.push(cron.schedule(config.shopQueue, processShopQueue));
=======
    // if login queue is enabled, process an item every 3 seconds
...
>>>>>>> upstream/master
  ...
  if (config.updateUserAgent)
    cronTasks.push(
      cron.schedule(config.updateUserAgent, fetchRiotClientVersion)
    );
};
...
<<<<<<< HEAD
  ...
  },
  {
    name: 'balance',
    description:
      'Show how many VALORANT Points & Radianite you have in your account!'
  },
  {
    name: 'alert',
      ...
      switch (interaction.commandName) {
        ...
        case 'bundle': {
          ...
          const searchQuery = interaction.options
            .get('bundle')
            .value.replace(/collection/i, '')
            .replace(/bundle/i, '');
          const searchResults = await searchBundle(
            searchQuery,
            interaction.locale,
            25
          );
          ...
          const nameMatchesExactly = (interaction) =>
            searchResults.filter(
              (r) =>
                l(r.obj.names, interaction).toLowerCase() ===
          ...
          if (searchResults.length === 0) {
            ...
            nameMatchesExactly(interaction) ||
            nameMatchesExactly()
          ) {
            // check both localized and english
            ...
            row.addComponents(
              new StringSelectMenuBuilder()
                .setCustomId('bundle-select')
                .setPlaceholder(s(interaction).info.BUNDLE_CHOICE_PLACEHOLDER)
                .addOptions(options)
            );
        ...
        case 'balance': {
          ...
          if (!balance.success)
            return interaction.followUp(
              authFailureMessage(
                interaction,
                balance,
                '**Could not fetch your balance**, most likely you got logged out. Try logging in again.'
              )
            );
          ...
          await interaction.followUp({
            embeds: [
              {
                // move this to embed.js?
                title: s(interaction).info.WALLET_HEADER.f(
                ...
                fields: [
                  {
                    name: s(interaction).info.VPOINTS,
                    value: `${theVPEmoji} ${balance.vp}`,
                    inline: true
                  },
                  {
        ...
        case 'update': {
          ...
          if (!authSuccess.success)
            return interaction.followUp(
              authFailureMessage(
                interaction,
                authSuccess,
                s(interaction).error.AUTH_ERROR_GENERIC
              )
            );
          ...
          await interaction.reply({
            embeds: [
              basicEmbed(
                s(interaction).info.ACCOUNT_UPDATED.f(
                  { u: user.username },
                  interaction
                )
              )
        ...
        case 'forget': {
          ...
          const targetAccount =
            interaction.options.get('account') &&
            interaction.options.get('account').value;
          if (targetAccount) {
            const targetIndex = findTargetAccountIndex(
              interaction.user.id,
              targetAccount
            );
            ...
            const usernameOfDeleted = deleteUser(
              interaction.user.id,
              targetIndex
            );
            ...
            await interaction.reply({
              embeds: [
                basicEmbed(
                  s(interaction).info.SPECIFIC_ACCOUNT_DELETED.f(
                    { n: targetIndex, u: usernameOfDeleted },
                    interaction
                  )
                )
              ]
        ...
        case 'stats': {
          ...
          if (skinName) {
            ...
            if (skins.length === 0) {
              ...
              skins.length === 1 ||
              l(skins[0].obj.names, interaction.locale).toLowerCase() ===
            ...
            await interaction.followUp(
              await allStatsEmbed(interaction, getOverallStats())
            );
          }
        ...
        case 'account': {
          ...
          await interaction.reply({
            embeds: [
              basicEmbed(
                s(interaction).info.ACCOUNT_SWITCHED.f(
                  { n: targetIndex, u: valorantUser.username },
                  interaction
                )
              )
            ]
