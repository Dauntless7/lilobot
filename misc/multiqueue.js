...
export const useMultiqueue = () =>
  config.useMultiqueue && client.shard && client.shard.ids[0] !== 0;
...
export const mqGetShop = async (id, account = null) =>
  await mqSendMessage('getShop', { id, account });
export const mqLoginUsernamePass = async (id, username, password) =>
  await mqSendMessage('loginUsernamePass', { id, username, password });
export const mqLogin2fa = async (id, code) =>
  await mqSendMessage('login2fa', { id, code });
export const mqLoginCookies = async (id, cookies) =>
...
const mqProcessRequest = async ({ mqid, mqtype, params }) => {
  ...
  switch (mqtype) {
    ...
    case 'loginUsernamePass': {
      ...
      response = await waitForAuthQueueResponse(
        await queueUsernamePasswordLogin(id, username, password)
      );
      break;
    }
    ...
    case 'login2fa': {
      ...
      response = await waitForAuthQueueResponse(
        await queue2FACodeRedeem(id, code)
      );
      break;
    }
    ...
    case 'loginCookies': {
      ...
      response = await waitForAuthQueueResponse(
        await queueCookiesLogin(id, cookies)
      );
      break;
    }
  }
