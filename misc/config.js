...
export const loadConfig = (filename = 'config.json') => {
  ...
  if (
    ...
    console.error(
      'The Profile command works without an HDEV token, but you can view up to 2 different accounts per hour.'
    );
    console.error(
      'If you need more than 2 accounts per hour, see https://discord.gg/B7AarTMZMK'
    );
    console.error(
      "If you don't want to see this notification again, set HDevTokenAlert to false in config.json file"
    );
  }
